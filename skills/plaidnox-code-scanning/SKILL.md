...
- Route every generative model call through the LiteLLM SDK. Keep provider/model choices in assets or deployment configuration. JEV makes typed routing and retrieval decisions; it does not scan code.
- Keep prompts, JEV questions, response schemas, model mappings, runtime policy, and SQL outside application code. Use strict Markdown prompt templates rendered with Jinja and standalone migrations.
...
- Use Perplexity Sonar through LiteLLM for current external security knowledge. Store only cited, source-backed knowledge. Research may guide a hunt but cannot confirm or close a finding.
