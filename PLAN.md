...
11. **Next:** use the computed delta/fanout to selectively execute only affected persisted investigations on incremental scans, while carrying forward unaffected verified findings with explicit source/graph evidence identity. Keep the current Graphify shadow/opt-in gate until parity, seeded-root recall, stale-finding invalidation, and checkpoint-resume behavior are tested; do not run an acceptance scan without an explicit user request.
