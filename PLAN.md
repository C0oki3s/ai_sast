...
8. **Next:** execute each persisted Graphify Investigation through the existing attacker-first hunt and independent Deep Hunt contracts. Bind candidate grounding to investigation source windows and graph references, checkpoint each investigation verdict, and reconcile this coverage independently from legacy discovery until parity. Preserve findings and coverage parity before making Graphify the default. Do not run real acceptance scans without explicit user instruction.
