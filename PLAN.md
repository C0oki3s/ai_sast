...
10. **Incremental invalidation mapping in progress:** `affected_investigation_ids()` maps changed/added/removed source files, graph nodes, and graph edges to investigations that explicitly depend on that evidence. It also invalidates a target when a changed edge touches its referenced node, while ignoring the repository-wide snapshot marker so unrelated investigations remain reusable. Focused delta tests are in `tests/test_graphify_adapter.py`. Next, wire this mapping into cross-snapshot reuse and add graph-derived reverse-dependency fanout before any investigation is reused after a revision change.
