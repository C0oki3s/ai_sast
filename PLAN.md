...
19. **Next:** exercise checkpoint-resume parity and seeded-root recall for Graphify-backed investigations, then compare invalidation and carry-forward behavior against a real disposable PostgreSQL instance before changing Graphify defaults. Do not run an acceptance scan without an explicit user request.
