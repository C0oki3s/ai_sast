...
21. **Disposable PostgreSQL migration and reuse verification implemented:** the opt-in test compose service now applies all checked-in Code Scanning migrations through `0010_graphify_snapshots.sql`. The full PostgreSQL persistence suite, including the complete-vs-legacy graph dependency carry-forward gate, passed against the isolated port-5434 tmpfs database; the service was then stopped and removed. No production/platform database was used. **Next:** add independent Deep Hunt gate coverage for the seeded identity root before changing Graphify defaults. Do not run an acceptance scan without an explicit user request.
