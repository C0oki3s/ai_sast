...
15. **Next:** expand safe finding reuse to unaffected graph deltas only after every finding persists complete graph-node/edge and source-symbol dependency identity, removed graph relationships fail closed, and evidence-location/prompt/context compatibility is checked. Keep Graphify opt-in until seeded-root recall, stale-finding invalidation, and checkpoint-resume parity are tested; do not run an acceptance scan without an explicit user request.
