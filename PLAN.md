...
13. **Exact-snapshot verified finding carry-forward implemented (opt-in):** only consider findings from a successful prior scan with the same prompt manifest and security-context scope, and only when the full Graphify snapshot identity is unchanged. The canonical finding must remain active (`validated`, `open`, or `in_progress`), use the supported report schema, and have every evidence path/range still admitted, present, and in bounds. The new scan writes a fresh scan-scoped report snapshot while preserving canonical proof and dependency rows. Same-source pipeline tests assert carry-forward; changed graph/source evidence yields no carry-forward. This deliberately does not yet reuse a finding across a graph delta, even when its local dependency hashes appear unchanged, because complete cross-surface graph dependencies are not yet represented on every finding.
14. **Removed source-symbol dependencies fail closed:** `findings_requiring_revalidation()` now flags findings whose recorded symbol dependency no longer exists in the new Security IR, in addition to changed symbols and their reverse callers. A persistence regression test removes a depended-on symbol and checks that its finding is invalidated. This only covers dependencies already recorded on the finding; it does not infer missing transitive edges.
15. **Graph investigation finding dependencies implemented:** findings originating in Graphify investigations now persist a typed dependency on that immutable investigation ID and Graphify snapshot. When delta analysis invalidates the investigation, its linked findings are flagged for revalidation before reuse. The pipeline regression test verifies the finding dependency is persisted and the linked finding is flagged when its investigation's source changes. Non-Graphify findings remain governed by their existing Security IR dependencies.
16. **Referenced Graphify finding dependencies implemented:** verified Graphify findings now persist dependencies on every referenced node, referenced edge, the complete observed neighborhood of each referenced node, and each admitted evidence source file. References added by context expansion travel with the candidate and enter the same dependency set. On later Graphify runs, changed or removed files, nodes, edges, or neighborhood relationships demote linked findings for revalidation; missing, malformed, or wrong-snapshot references persist an unresolved dependency that fails closed on reuse. Regression tests prove edge removal invalidates a finding even when its file and node hashes are unchanged, and the pipeline test verifies graph node/neighborhood/source-file dependencies are saved. Full test suite and Ruff pass. No real scan was run; Graphify remains opt-in.
17. **Dependency-checked finding carry-forward across Graphify deltas implemented (opt-in):** after graph-based revalidation demotes findings whose recorded source, node, neighborhood, edge, or investigation dependencies changed or disappeared, a successful prior report may be carried across an unrelated graph delta only when the finding was persisted with dependency contract version 1, remains active, and passes the existing workflow, context-scope, report-schema, source-admission, and evidence-range checks. Exact-snapshot reuse remains compatible with older persisted records. Findings with absent, malformed, or stale graph references store an unresolved dependency and cannot be carried across a delta. The behavior is separately controlled by `reuse_verified_findings_across_graph_deltas`; a pipeline regression test proves an unrelated file addition reuses the finding while a changed depended-on file still forces a new hunt. Full test suite and Ruff pass. No real scan was run; Graphify remains opt-in.
18. **Reuse eligibility audit and PostgreSQL integration coverage implemented:** reports now expose whether cross-delta reuse required graph dependencies, how many prior reports were eligible, how many were rejected for missing dependency metadata, and the dependency kinds supporting reuse. A disposable-PostgreSQL integration test covers successful-scan/report persistence and verifies that only complete dependency-contract records pass the cross-delta gate; it is safely skipped unless `PLAIDNOX_TEST_DATABASE_URL` is explicitly configured. SQLite pipeline/persistence regressions, the full suite, Ruff, and diff checks pass. No live PostgreSQL target was configured, so the database integration test was not executed.
19. **Pipeline checkpoint-resume parity implemented:** a pipeline integration regression simulates a verifier failure after a Graphify investigation result has been checkpointed, reruns the same immutable snapshot, and verifies the grounded candidate is replayed without another hunt invocation while the failed verifier work retries and succeeds. Existing agent-level tests separately cover validated-result replay and stale-source rejection. This is a fake-gateway test; no real scan or model provider was used. Full suite and Ruff pass.
20. **Seeded verified-identity root grounding regression implemented:** a deterministic Graphify fixture models the verified Cognito middleware, attacker-controlled `x-user-email` claim overwrite, route-to-middleware attachment, and protected account lookup. A fake gateway returns the expected cross-account hypothesis; the test verifies exact root-cause lines, snapshot-bound node/edge references, invariant and gained-capability fields, and terminal obligation disposition through the production hunt response validator. This checks the seeded root's semantic contract and grounding without claiming live-model recall. Full suite and Ruff pass; no scan or provider call was run.
21. **Disposable PostgreSQL and seeded-root independent verification gates implemented:** the opt-in test compose service applies all Code Scanning migrations through `0010_graphify_snapshots.sql`; its full persistence suite passed on the isolated port-5434 tmpfs database and the service was removed. The seeded verified-identity case now also passes through the independent Deep Hunt validator only when all eight gates pass, and a contradictory `effective_defense=fail` response is rejected. Together with the Graphify grounding regression, the test covers source identity overwrite, the protected account lookup, and fail-closed independent verification without live model calls. **Next:** keep Graphify opt-in until a user-authorized acceptance scan checks seeded-root recall, checkpoint resume, and cost on NSTCTF; do not run that scan without an explicit request.
22. **Graphify structural IDs survive duplicate-symbol insertion and checkout changes:** normalize Graphify's root-qualified structural identifiers into portable PlaidNox IDs so same-named methods remain tied to their structural parent when earlier duplicates are inserted. Include the adapter identity contract in `extractor_version`, ensuring persisted snapshot identity changes when the normalization algorithm changes. A real local Graphify extraction regression checks duplicate methods, insertion, line movement, and distinct checkout roots. Full tests, Ruff, compilation, and diff checks pass. This improves Graphify migration identity correctness only; it does not enable Graphify by default or run a source scan.
23. **External Graphify symbol placeholders remain explicit unresolved edges:** the pinned structural extractor emits import/inheritance target nodes with no source path or location, including repeated placeholder IDs. The adapter now omits only those location-free external nodes, preserves source-grounded edges as unresolved relationships, and still rejects partially malformed source locations and duplicate in-snapshot IDs. A local import fixture and a real extraction of the Code Scanning package confirm extraction completes with unresolved-edge and unindexed-file counts exposed. This is structural indexing, not a vulnerability scan; runtime adoption remains gated on the authorized acceptance suite.
24. **Reverse-invalidation bounds fail closed for one-shot iterables:** `affected_investigation_ids()` accepts general iterables, so it now materializes investigation records once before the normal invalidation pass and the bounded-overflow fallback. A regression passes a generator while forcing reverse-depth exhaustion and confirms every investigation is invalidated. Focused and full test suites, Ruff, compilation, and diff checks pass; no source scan was run.
