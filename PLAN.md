...
4. **First Graphify AI-planner slice implemented:** `GraphInvestigationPlanner` builds a size-bounded neighborhood around an explicit target, provides redacted exact source windows and typed graph provenance, asks the configured LiteLLM agent for open-ended security questions, and rejects any invented node/edge references. Full security-surface enumeration, batched target planning, coverage reconciliation, checkpoint/ledger integration in the scan runtime, and Deep Hunt execution from investigations remain.
