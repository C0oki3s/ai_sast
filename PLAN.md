...
18. **Next:** exercise cross-snapshot carry-forward and fail-closed dependency migration against PostgreSQL, add explicit audit telemetry for which dependency kinds admitted or rejected reuse, then evaluate checkpoint-resume and seeded-root parity before changing Graphify defaults. Do not run an acceptance scan without an explicit user request.
