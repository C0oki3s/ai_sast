...
16. **Next:** persist full referenced Graphify node/edge/neighborhood dependencies for findings, including dependencies acquired during context expansion; invalidate them on removed or changed relationships and fail closed when evidence cannot be mapped. Only then broaden verified-finding carry-forward beyond identical graph snapshots. Keep Graphify opt-in until seeded-root recall, stale-finding invalidation, and checkpoint-resume parity are tested; do not run an acceptance scan without an explicit user request.
