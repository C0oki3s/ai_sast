...
17. **Next:** permit verified-finding carry-forward across a changed Graphify snapshot only when all recorded source-symbol, source-file, graph-node, graph-edge, graph-neighborhood, investigation, prompt, context-scope, report-schema, and evidence-admission checks pass and the prior successful scan's finding remains active. Legacy findings without complete Graphify dependency records must be re-hunted. Keep Graphify opt-in until seeded-root recall, stale-finding invalidation, and checkpoint-resume parity are tested; do not run an acceptance scan without an explicit user request.
