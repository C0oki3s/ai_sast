...
20. **Next:** add seeded-root semantic recall tests for Graphify-backed investigations, then compare invalidation and carry-forward behavior against a real disposable PostgreSQL instance before changing Graphify defaults. Do not run an acceptance scan without an explicit user request.
